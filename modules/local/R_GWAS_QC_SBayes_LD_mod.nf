process R_GWAS_QC_SBayes_LD {
    ...
    tuple val(cohort), path (GWAS_QC_noclump), \
        ...
        path(private_input_files_path),path(blocks_files_path)
    
    

    output:
    // $outDir/ldm.info, $outDir/snp.info, $outDir/block*.eigen.bin 
    tuple val(cohort), path ("${cohort}_LD/*"), path("${cohort}_LOO_GWAS_QC_noclump.cojo"), emit: SBayes_LD
    
    
    script:
    """
    
    R_GWAS_QC_SBayes_LD.R ${cohort} ${GWAS_QC_noclump}  ${private_input_files_path} ${blocks_files_path}
